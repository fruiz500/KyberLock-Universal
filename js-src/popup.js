  ...
  function(request, sender, sendResponse) {
	...
	//now get the serial from sync storage, and put it in the first serial box, and the userID in its place
	  ...
	  //close everything and erase cached Master Password and encrypted stuff after five minutes
		...
		}else{									//no passwords to be filled, so open the regular KyberLock interface, with the main box filled	
			var length = request.KLstuff.length;
			...
			if(request.largeInputs > 0){			//there are fillable boxes, so open the compose interface
				...
				if(length > 1){
					if(websiteName == 'google.com' || websiteName == 'live.com' || websiteName == 'yahoo.com'){
						text2decrypt = request.KLstuff[0]				//line breaks must be preserved. Important for text stego
					}else{
						text2decrypt = confirm('It seems there is more than one crypto item on the page. If you click OK, the first will be taken, otherwise the last will be taken.') ? request.KLstuff[0] : request.KLstuff[length - 1]
					}
				}else{
					text2decrypt = request.KLstuff[0]
				}
				clearTimeout(startTimer);
				callKey = 'decrypt';
...
function fetchWebsiteMemo(){
	...
	//the rest is to decrypt an encrypted note		
			if(memoCipher){
				...
				var cipher = decodeBase64(memoCipher);
				if(!cipher) return false;

				var	nonce = cipher.slice(0,24),												//no marker byte
					cipher2 = cipher.slice(24),
					pwd2 = wiseHash(masterPwd,websiteName,32);
				try{
					plain = nobleCiphers.xchacha20poly1305(pwd2,nonce).decrypt(cipher2);
					preserveMaster();									//sync only if successful
					memoBox.value = encodeUTF8(plain)
				}catch{
...
//called if there is no response from the content script or there is no KyberLock or SynthPass function to do
function showMemo(name){
	...
	clipbdBtn.style.display = 'none';
	synthTitle.textContent = "KyberLock notes";
...
function preserveMaster(){
	if(masterPwd){
		...
		chrome.alarms.create("KLUAlarm",						//to delete all in session storage after 5 minutes
			{"delayInMinutes": 5}
		);
	}
}
...
function preserveKeys(){
	...
	if(mySeed) chrome.storage.session.set({"mySeed": JSON.stringify(Array.from(mySeed))});
...
function retrieveKeys(){
	...
	gettingKeys.then(function(result){
		...
		if(result["mySeed"]){
			mySeed = new Uint8Array(JSON.parse(result["mySeed"]));
		...
		if(websiteName){
			...
			if(!memoBox.value.trim()){					//nothing to encrypt, so save an empty string
				...
				var	nonce = crypto.getRandomValues(new Uint8Array(24)),
					plain =	 decodeUTF8(memoBox.value.trim()),
					pwd2 = wiseHash(masterPwd,websiteName,32),
					cipher = nobleCiphers.xchacha20poly1305(pwd2,nonce).encrypt(plain),
					cipherStr = encodeBase64(concatUi8([nonce,cipher])).replace(/=+$/,'');
			}
			var jsonfile = {};
			jsonfile[websiteName] = [serial1.value,userID.value,pwdLength.value,cryptoStr,cipherStr];
    		if(isEmptyJSON(jsonfile)){
				chrome.storage.sync.remove(websiteName)
			}else{
    			chrome.storage.sync.set(jsonfile)
...
function pwdSynth(boxNumber, pwd, serial, isPin, isAlpha){
	...
	if(serial == '-'){						//special case to delete stored password, and using Master Password directly in a change password case
		...
		if(cryptoStr && (boxNumber == 1 || !document.getElementById('serial' + boxNumber).value)){						//already stored in encrypted form: decrypt it
			var cipher = decodeBase64(cryptoStr);
			if(!cipher) return false;
			var	nonce = cipher.slice(0,24),												//no marker byte
				cipher2 = cipher.slice(24),
				pwd2 = wiseHash(pwd,websiteName,32),
				plain = nobleCiphers.xchacha20poly1305(pwd2,nonce).decrypt(cipher2);
			if(plain){
			...
			if(!sitePwd){
				...
				extraMasterAction = 'decrypt';
				var	nonce = crypto.getRandomValues(new Uint8Array(24)),
					plain =	 decodeUTF8(sitePwd),
					pwd2 = wiseHash(pwd,websiteName,32),
					cipher = nobleCiphers.xchacha20poly1305(pwd2,nonce).encrypt(plain);
				cryptoStr = encodeBase64(concatUi8([nonce,cipher])).replace(/=+$/,'')
				return sitePwd
			}
		}
	//the rest of the options are synthesized through wiseHash
		...
		cryptoStr = '';
		return encodeBase64(wiseHash(pwd,websiteName + serial.trim(),32)).replace(/[AaBbC]/g,'0').replace(/[cDdEe]/g,'1').replace(/[FfGgH]/g,'2').replace(/[hIiJj]/g,'3').replace(/[KkLlM]/g,'4').replace(/[mNnOo]/g,'5').replace(/[PpQqR]/g,'6').replace(/[rSsTt]/g,'7').replace(/[UuVvW]/g,'8').replace(/[wXxYy]/g,'9').match(/[0-9]/g).join('')
	}else if(isAlpha){						//replace extra base64 characters with letters
		cryptoStr = '';
		return encodeBase64(wiseHash(pwd,websiteName + serial.trim(),32)).replace(/\+/g,'a').replace(/\//g,'b').replace(/=/,'c')
	}else{
		cryptoStr = '';
		if(base == base62){				//replace some base64 characters with default special characters
			return encodeBase64(wiseHash(pwd,websiteName + serial.trim(),32)).replace(/[+/=Aa]/g,'_').replace(/[BbCc]/,'!').replace(/[DdEe]/,'#')
		}else{								//change base in order to include the special characters, with equal probability
			return base.charAt(62) + changeBase(encodeBase64(wiseHash(pwd,websiteName + serial.trim(),32)).replace(/=$/g,''), base64, base) 				//use at least the first of the characters on the list
		}
	}
}
