...
function moveDB(){
	if(composeBox.innerHTML.match('href="data:,==k') && wipeEnabled){			//delete data the second time
		...
		fileLink.download = "KLbak.txt";
		fileLink.href = 'data:,==' + keyEncrypt(JSON.stringify(locDir)) + '==';
		fileLink.textContent = "KyberLock encrypted database; right-click and Save link as...";
...
function fillRecipients(){
	...
	if(emailList.length){
		...
		composeRecipientsBox.textContent = "Nobody!.. Making an invitation or a Backup"
	}
	if(lockList.options[0].selected){
		composeRecipientsBox.textContent = "Unspecified... Using Shared Key mode";
