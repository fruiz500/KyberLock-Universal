...
function insertInBody(node){
	chrome.tabs.sendMessage(activeTab.id, {message: "clicked_OK", KLoutput: node.innerHTML});
}
...
function openReadScreen(){
	if(readScr.style.display != 'block') openScreen('readScr')
}
...
//		readInterfaceBtn.innerHTML = '&#9668;';
		...
		readMsg.textContent = "Click the second button to decrypt a file or an image, the third to save files in the box, the fourth to reveal a hidden message, the fifth to display your Lock\r\nThe sender's Lock fingerprint is:  " + lockPrint(decodeBase64(theirLock))
	}else{
		moreReadButtons.style.display = 'none';
//		readInterfaceBtn.innerHTML = '&#9658;';
		readInterfaceBtn.textContent = '\u25BA';
		readMsg.textContent = "Click the arrow to decrypt attachments, reveal a hidden message, or display your Lock or the sender's fingerprint"
	}
}
...
function updateComposeButtons(){
	if(emailList.length == 0){									//display invite button for no recipients
		encryptBtn.style.display = 'none';
		moreComposeButtons.style.display = 'none';
		inviteBtn.style.display = '';
		interfaceBtn.style.display = 'none';
		checkBoxes.style.display = 'none';
		composeMsg.textContent = 'No known recipients selected, so an invitation wil be made\r\nThe contents WILL NOT BE SECURE\r\n(You can also make a backup if text is empty)'
	}else{
		...
		if(encodeURI(interfaceBtn.textContent) == "%E2%96%BA"){
			...
			checkBoxes.style.display = 'none';
			checkBoxes.style.display = 'none';
			composeMsg.textContent = 'Type your message and click Encrypt to Email. More options with the arrow'
		}else{
			moreComposeButtons.style.display = '';
			checkBoxes.style.display = '';
			checkBoxes.style.display = '';
			composeMsg.textContent = 'Type your message, set options, and click an Encrypt button'
		}
	}
}
