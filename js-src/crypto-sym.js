...
function symmetricEncrypt(text,isFileOut,isImageOut){
	if(!sharedPwd){											//open shared key input if there's no password in memory
		symmetricMsg.textContent = 'Enter the shared Key';
		symmetricMsg.style.color = '';
		openScreen('symmetricScr');
		return
	}
	
	if(sharedPwd.length > 500 && !isBase64(sharedPwd)){					//special mode for long keys, first cut
	...
	var sharedKey = wiseHash(sharedPwd,nonceStr,32);		//use the nonce for stretching the user-supplied Key

	var cipher = symEncrypt(text,nonce,sharedKey,true),		//true because compression is used
		outStr = encodeBase64(concatUi8([[128],nonce,cipher])).replace(/=+$/,'');
...
function symmetricDecrypt(cipherStr){
	...
	if(cipherStr.charAt(0) == 'd' && sharedPwd.length > 500){ 					//special mode for long keys
	...
	var	nonce = fullArray.slice(1,25),
		cipher = fullArray.slice(25);

	var sharedKey = wiseHash(sharedPwd,encodeBase64(nonce),32);				//real shared Key
	sharedPwd = '';

	var plain = symDecrypt(cipher,nonce,sharedKey,true);			//if this fails, the function posts a generic message
	if(!plain){
		failedDecrypt('');
		openReadScreen();
		return
	...
	readMsg.textContent = 'shared Key decryption successful';
	openReadScreen();
	callKey = ''
}
...
function padResult(textBin, keyTextBin, nonce, startIndex){
	...
	for(var index = 0; index < count; index++){
		...
		var hash = nobleHashes.sha512(inputArray);			//now take the hash
		for(i = 0; i < 64; i++){
