...
function pwdSynth(pwd, serial, isPin, isAlpha){
	if(serial == '-' || serial == '+'){
		...
		return encodeBase64(wiseHash(pwd,websiteName + serial,32)).replace(/[AaBbC]/g,'0').replace(/[cDdEe]/g,'1').replace(/[FfGgH]/g,'2').replace(/[hIiJj]/g,'3').replace(/[KkLlM]/g,'4').replace(/[mNnOo]/g,'5').replace(/[PpQqR]/g,'6').replace(/[rSsTt]/g,'7').replace(/[UuVvW]/g,'8').replace(/[wXxYy]/g,'9').match(/[0-9]/g).join('')
	}else if(isAlpha){						//replace extra base64 characters with letters
		return encodeBase64(wiseHash(pwd,websiteName + serial,32)).replace(/\+/g,'a').replace(/\//g,'b').replace(/=/,'c')
	}else{
		if(base == base62){				//replace some base64 characters with default special characters
			return encodeBase64(wiseHash(pwd,websiteName + serial,32)).replace(/[+/=Aa]/g,'_').replace(/[BbCc]/,'!').replace(/[DdEe]/,'#')
		}else{								//change base in order to include the special characters, with equal probability
			return base.charAt(62) + changeBase(encodeBase64(wiseHash(pwd,websiteName + serial,32)).replace(/=$/g,''), base64, base) 				//use at least the first of the characters on the list
		}
	}
}
...
function hashili(msgID,string){
	...
	hashiliTimer = setTimeout(function(){
		if(!string.trim()){
			...
			var code = nobleHashes.sha512(decodeUTF8(string.trim())).slice(-2),			//take last 2 bytes of the SHA512
