...
var noblePostQuantum = (() => {
  ...
  var polyCoder2 = (d, compress2) => bitsCoder2(d, {
  ...
  function getDilithium(opts2) {
    ...
    const ETACoder = polyCoder2(ETA === 2 ? 3 : 4, (i) => ETA - i);
  ...
  function gen2(opts2, hashOpts) {
    ...
    const setAddr = (opts3, addr = new Uint8Array(ADDR_BYTES)) => {
      const { type, height, tree, layer, index, chain, hash: hash2, keypair } = opts3;
  ...
  var genSha = (h0, h1) => (opts2) => (pub_seed, sk_seed) => {
    ...
    function mgf1(seed, length, hash2) {
      stats.mgf1++;
      const out = new Uint8Array(Math.ceil(length / hash2.outputLen) * hash2.outputLen);
      if (length > 2 ** 32)
        throw new Error("mask too long");
      for (let counter = 0, o = out; o.length; counter++) {
        counterV.setUint32(0, counter, false);
        hash2.create().update(seed).update(counterB).digestInto(o);
        o = o.subarray(hash2.outputLen);
      }
